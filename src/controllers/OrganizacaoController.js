...
class OrganizacaoController {
  static async create(req, res) {
    try {
      const new_user_data = await OrganizacaoService.create(req.body);
  ...
  static async get_by_id(req, res) {
    try {
      const data = await OrganizacaoService.get_by_id(req.params.id);
  ...
  static async get_all(req, res) {
    try {
      const data = await OrganizacaoService.get_all();
      return res.status(200).json(data);
    } catch (error) {
  ...
  static async update(req, res) {
    try {
      const data = await OrganizacaoService.update(
        req.params.id,
        req.body.updates
      );
      return res.status(200).json(data);
  ...
  static async delete(req, res) {
    try {
      await OrganizacaoService.delete(req.params.id);
