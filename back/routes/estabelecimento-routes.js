...
router.get('/:id', async (req, res) => {
    try {
        const estabelecimento = await EstabelecimentoService.buscarPorId(req.params.id);
...
router.put('/:id', async (req, res) => {
    try {
        const estabelecimento = await EstabelecimentoService.atualizar(req.params.id, req.body);
        res.status(200).json(estabelecimento);
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
