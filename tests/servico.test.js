...
describe("Testes de Integração - Serviço", () => {
  ...
  test("Deve criar um servico via API", async () => {
    const res = await request(app).post("/servicos").send(novoServico);
  ...
  describe("Leitura", () => {
    test("Deve listar o servico criado pelo ID", async () => {
      const res = await request(app).get(`/servicos/${servicoCriadoId}`);
  ...
  describe("Exclusão", () => {
    test("Deve excluir o servico criado", async () => {
      const res = await request(app).delete(`/servicos/${servicoCriadoId}`);
