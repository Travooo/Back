...
describe("Testes de Integração - Pagamentos", () => {
  ...
  describe("Leitura", () => {
    test("Deve buscar um pagamento por ID", async () => {
      const res = await request(app).get(`/pagamentos/${pagamentoCriadoId}`);
    ...
    test("Deve buscar o status de um pagamento", async () => {
      const res = await request(app).get(
        `/pagamentos/${pagamentoCriadoId}/status`
      );
  ...
  describe("Atualização", () => {
    test("Deve atualizar o status de um pagamento", async () => {
      ...
      const res = await request(app)
        .put(`/pagamentos/${pagamentoCriadoId}/status`)
        .send(update);
    ...
    test("Deve atualizar um pagamento existente", async () => {
      const res = await request(app)
        .put(`/pagamentos/${pagamentoCriadoId}`)
  ...
  describe("Exclusão", () => {
    test("Deve deletar um pagamento", async () => {
      const res = await request(app).delete(`/pagamentos/${pagamentoCriadoId}`);
